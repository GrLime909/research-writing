---
name: wos-search
description: Search Web of Science by topic, author, title, DOI, or advanced query. Supports edition/database filtering and sort. Journal list filtered via wos-journals.txt.
...
## Journal Filtering (Mandatory)

**Before any search**, read `wos-journals.txt` from the skill root directory and
build an `SO` clause. The journal list is maintained in that external file —
never hardcoded.

```
SO=("Journal Name 1" OR "Journal Name 2" OR ... OR "Journal Name N")
```

This clause becomes the **first row** of every WOS query, with subsequent
keyword rows connected by `AND`.
...
```json
{
  ...
  "search": {
    ...
    "database": "WOSCC",
    "query": [
      {"rowField": "SO", "rowText": "JOURNAL_FILTER"},
      {"rowBoolean": "AND", "rowField": "TS", "rowText": "USER_QUERY"}
    ],
    "editions": ["WOS.SSCI"]
  },
  "retrieve": {
