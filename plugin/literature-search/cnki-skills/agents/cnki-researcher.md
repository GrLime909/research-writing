...
When encountered:
1. **Stop** immediately.
2. **Notify**: "CNKI 正在显示滑块验证码。请�?Chrome 中手动完成拼图验证，完成后告诉我继续�?
...
### cnki-search �?文献检�?Search CNKI for papers by keyword.
...
### cnki-journal-search �?期刊检�?Search for journals by name, ISSN, or CN number.
...
### cnki-navigate-pages �?翻页与排�?Navigate search result pages or change sort order.
- `/cnki-navigate-pages next|previous|page N|sort by date|citations|downloads`

### cnki-advanced-search �?高级检�?Advanced search with field filters (author, title, journal, date range, source category).
...
### 1. Literature Search (文献检�?
```
User: "搜索关于 transformer 的论�?
...
### 3. Journal Search (期刊检�?
```
User: "帮我找《计算机学报�?
�?cnki-journal-search "计算机学�?
...
�?cnki-journal-index "计算机学�?
�?Present: 收录数据�?(北大核心, EI, CSCD, ...), 影响因子, 基本信息
```
...
5. **Check login status.** If downloads show "未登�?, remind user to log in.
