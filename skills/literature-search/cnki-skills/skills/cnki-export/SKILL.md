...
## Mode C: Export to Excel (论文清单)

When user wants to save search results as an Excel file instead of Zotero.

### Excel Columns

| 列名 | 来源 | 说明 |
|------|------|------|
| 论文名 | 搜索结果页 `td:nth-child(2)` | 去除"网络首发"后缀 |
| 期刊 | 搜索结果页 `td:nth-child(4)` | |
| 发表日期 | 搜索结果页 `td:nth-child(5)` | |
| 摘要 | 逐篇抓取 or 导出API | 可选，需防验证码 |
| 作者 | 逐篇抓取 or 导出API | 可选 |
| 关键词 | 逐篇抓取 or 导出API | 可选 |
| 入库时间 | `datetime.now()` | 自动生成 |
...
# 从 /tmp/cnki_results.json 读取已筛选的结果列表
# 列: 论文名、期刊、发表日期、入库时间
```

### Full Export (含摘要/作者/关键词)

需对每篇论文调用 CNKI 导出 API (`dm8/API/GetExport`)，
从 ENDNOTE 格式中解析摘要、作者、关键词。
注意：批量调用需加延迟防止验证码。

### Workflow 整合

与 `cnki-advanced-search` 配合使用：
1. `cnki-advanced-search` → 执行搜索 + 期刊列表后过滤
2. `cnki-export` (Mode C) → 导出为 Excel
