...
| 技能 | 功能 | 触发方式 |
|------|------|----------|
| wos-search | 关键词搜索（含期刊过滤 + 关键词扩展） | `/wos-search {keywords}` |
| wos-parse-results | 解析当前搜索结果页 | 自动调用 |
| wos-navigate-pages | 翻页与排序 | `/wos-navigate-pages next` |
| wos-paper-detail | 提取论文完整元数据 | `/wos-paper-detail {WOS ID}` |
| wos-download | 下载论文 PDF | `/wos-download {WOS ID}` |
| wos-export | 导出到 Zotero / RIS / BibTeX | `/wos-export` |
| wos-excel-export | 导出到 Excel（含去重、翻译占位） | `/wos-excel-export` |
...
hardcoded in any SKILL.md — the `.txt` file is the single source of truth.
